...
namespace backend_canvas
{
    ...
    public static class MessageService
    {
        ...
        /// <summary>
        /// Take all received ServerMessages responses from the server if are any
        /// </summary>
        /// <returns>Server responses</returns>
        public static ServerMessage[] GetServerResponseMessages()
        {
            if (_responseQueue.Count == 0)
                return Array.Empty<ServerMessage>();

            return (ServerMessage[])_responseQueue.ToArray();
        }
