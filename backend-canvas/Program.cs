...
namespace backend_canvas
{
	internal class Program
	{
		static async Task Main(string[] args)
		{
			...
			MessageReceivingService wsl = new MessageReceivingService(clientManager.Credentials);
			await wsl.StartService();
			await clientManager.SendMessageAsync(new Flare.V1.DiffieHellmanMessage(), clientManager.Username);
